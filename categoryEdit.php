...
$query = "SELECT * FROM categories WHERE CategoryID = $id";
   ...
   if(isset($_POST["edit"])){
    ...
    $result = mysqli_query($conn,$query);
    if(isset($result)){
        header("location:category.php?msg=updated database successfuly");
    }
    else{
        echo "error";
    }
...
y
    ...
    <div class="sign-up m-5 p-5">
        ...
        <form  method="POST" class="w-50">
            <div class="mb-3">
                <label for="Name" style="font-size: 12px;" class="form-label">Edit Category Name</label>
                <input type="text" style="width: 100%;" class="form-control border-bottom border-0 border" id="Name" name="name" value="<?php echo $row['CategoryName']?>"  aria-describedby="emailHelp">
                <input type="hidden" id="Name" name="editeId" value="<?php echo $id?>" >
            </div>
        
            <button type="submit" class="btn text-white rounded-pill btn-outline-primary" style="background-color: #3498DB; width:164px; height:39px" id="edit" name= "edit">Edit</button>
        </form>
    </div>
</section>
