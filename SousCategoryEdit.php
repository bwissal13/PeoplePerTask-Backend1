...
if (isset($_POST["submit"])) {
    $souscategory = $_POST["name"];
    $query = "UPDATE souscategories SET SousCategoryName=' $souscategory' WHERE SousCategoryID = $id";
    $result = mysqli_query($conn, $query);

    if ($result) {
        header("location:sousCategories.php?msg=updated database successfully");
        exit();
    } else {
        echo "Error: " . mysqli_error($conn);
    }
}
$query = "SELECT * FROM souscategories WHERE SousCategoryID= $id";
$result = mysqli_query($conn, $query);
...
<section class="container-sign" id="main">
    <div class="sign-up m-5 p-5">
        <h2>Edit Sous Category</h2>
        <form action="" method="POST" class="w-50">
            <div class="mb-3">
                <label for="Name" style="font-size: 12px;" class="form-label">Sous Category Name</label>
                <input type="text" style="width: 100%;" class="form-control border-bottom border-0 border" id="Name" name="name" value="<?php echo $row['SousCategoryName']; ?>"  aria-describedby="emailHelp">
            </div>
            <div>
                ...
                <select class="form-select border-bottom border-0 border mb-5" aria-label="Default select example" name="category">
                    ...
                    <?php
                        ...
                        foreach ($categories as $category) {
                            echo "<option value='$category' " . ($category == $row['CategoryName'] ? 'selected' : '') . ">$category</option>";
                        }
